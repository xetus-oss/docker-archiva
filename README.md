...
A few manual steps may be required to upgrade to `v2` series of the `xetusoss/archiva`, depending on your deployment configuration.

##### If you use the pre v2 default Archiva configuration

_The `v2` and later images will fail to start if this applies to you._

The container may need to perform some potentially destructive upgrade steps if you've retained the original default Archiva configuration file generated by a pre-`v2` image. The upgrade will:

1. Replace all `location` and `indexDir` elements with relative paths in your `/archiva-data/conf/archiva.xml` configuration with their correct absolute paths;
2. Remove the JCR at `/archiva-data/data/jcr`, which Archiva uses to store metadata about all artifacts;
3. Remove the `/archiva-data/repositories/repositories` directory, which contains index directories for all repositories.

Once the upgrade is complete Archiva will need to completely re-scan and re-index all repositories, which may take some time depending on the size of your Archiva repositories.

If you have any concerns about this (e.g. you have a repository named `repositories`), **please** make backups of these three mount paths before performing the upgrade:

* `conf/archiva.xml`
* `data/jcr`
* `repositories/repositories`

When ready to perform the upgrade, start the container with the `UPGRADE_PRE_V2` environment variable set to `true`. For example:

```bash
docker run -d --name archiva \ 
              -v /host/path/archiva-data:/archiva-data \ 
              -e UPGRADE_PRE_V2=true \ 
              xetusoss/archiva:v2
```
